...
// ═══════════════════════════════════════════════════════
let user = null;
let entries  = new Map(); // Stocke les données du calendrier : "YYYY-MM-DD" -> {status, note, ...}
let cellCache = new Map(); // Cache des éléments DOM pour mise à jour rapide
let pendingImport = [];
...
function setupPWA() {
  window.addEventListener('beforeinstallprompt', e => { 
    e.preventDefault(); 
    pwaInstallPrompt = e; 
    if($('pwaInstallRow')) $('pwaInstallRow').style.display = 'block'; 
  });
  ...
  $('btnInstallPWA')?.addEventListener('click', async () => {
    ...
    if (outcome === 'accepted') { 
      if($('pwaInstallRow')) $('pwaInstallRow').style.display = 'none'; 
...
async function loadEntries() {
  if (!user) return;
  // Charger une plage large pour éviter les requêtes multiples
...
function renderGrid() {
  ...
  // Ajustement pour commencer la semaine Lundi (0=Dimanche -> 6, 1=Lundi -> 0)
  ...
  for (let i = 0; i < 42; i++) {
    ...
    if (entry?.status && entry.status !== 'repos') {
      ...
      lbl.textContent = entry.custom_label || STATUS_LABELS[entry.status]; 
...
function updateStats() {
  ...
  $('statJour').textContent = j; 
  $('statNuit').textContent = n; 
  $('statRepos').textContent = r;
...
async function saveEntry(k, patch) {
  if (!user) { 
    const g=$('gate'); 
    if(g){g.style.display='grid';g.classList.add('show');} 
    return; 
  }
  ...
  if (patch.status===null) entries.delete(k); 
  else entries.set(k, next);
  
  // Mise à jour visuelle immédiate
  const cell = cellCache.get(k);
  if (cell) {
    cell.className = ['day', 
      cell.classList.contains('out')?'out':'', 
      cell.classList.contains('selected')?'selected':'', 
      cell.classList.contains('today')?'today':'', 
      next.status||'', 
      next.imported?'imported':''
    ].join(' ');
    
    cell.innerHTML = `<span class="day-num">${parseKey(k).getDate()}</span>`;
    if (next.status && next.status!=='repos') {
      cell.innerHTML += `<span class="day-label">${next.custom_label||STATUS_LABELS[next.status]}</span>`;
    }
  ...
  try {
    if (patch.status===null) {
      ...
      // Gestion erreur spécifique si la colonne imported pose problème
      if(error && error.message.includes('imported')) { 
        delete p.imported; 
        await supabase.from("work_calendar_entries").upsert(p,{onConflict:"user_id,work_date"}); 
      }
    }
  } catch(e) { 
...
function handleFile(e) {
  ...
  reader.onload = evt => {
    try {
      ...
      // Trouver la feuille la plus remplie
...
function importAnalyze(rows) {
  ...
  if(!dateRows.length) {showToast("Aucune date détectée");return;}
  
  const agents = detectAgents(rows, dateRows);
  if(!agents.length) {showToast("Aucun agent détecté");return;}
  
  // Pré-sélectionner l'agent si le nom correspond aux préférences
  let target = prefs.agentName ? agents.find(a=>normalize(a.name).includes(normalize(prefs.agentName))) : null;
  
  if(target) {
    showImportPreview(buildServicesForAgent(target, dateRows, rows), agents, target.name);
  } else {
...
function findDateRows(rows) {
  ...
  rows.forEach((row, idx) => {
    ...
    row.forEach((c, i) => {
      ...
      if(m) { 
        map[i]=new Date(state.year, parseInt(m[2])-1, parseInt(m[1])); 
        cnt++; 
      }
      else if(/^\d{1,2}$/.test(v) && v<=31) { 
        map[i]={dayOnly:parseInt(v)}; 
        cnt++; 
      }
    });
...
function detectAgents(rows, dRows) {
  ...
  rows.forEach((r,i) => {
    ...
    r.forEach(c => {
      const v=String(c).trim();
      // Détection nom (lettres, espaces, tirets, min 3 car)
      if(v.length>3 && /^[A-Za-zÀ-ÿ\s\-]+$/.test(v)) {
        const w=v.split(/\s+/).filter(x=>x.length>1);
        if(w.length>=2 && !ags.find(a=>normalize(a.name)===normalize(v))) {
          ags.push({name:v, rowIdx:i});
        }
      }
    });
  });
...
function buildServicesForAgent(ag, dRows, rows) {
  // Trouver la ligne de date la plus proche au-dessus de l'agent
  ...
  // Mapper les colonnes aux dates
  Object.entries(dR.dateMap).forEach(([i,v]) => { 
    cmap[parseInt(i)] = (v instanceof Date)?v:new Date(state.year,m,v.dayOnly); 
  });
  
  const svcs=[], used=new Set();
  // Scanner les 4 lignes sous le nom de l'agent
  for(let k=0;k<=3;k++) {
    const row = rows[ag.rowIdx+k]; 
    if(!row) continue;
    row.forEach((c, i) => {
      ...
      let d=cmap[i]; 
      // Si pas de date directe, prendre la précédente
      if(!d) for(let j=i;j>=0;j--) if(cmap[j]){d=cmap[j];break;}
      
      if(!d) return;
      const key=keyFor(d); 
      if(used.has(key)) return;
      
      const st = interpretCode(raw); 
      if(!st) return;
...
function interpretCode(r) {
  ...
  if(u.includes("NUIT")) return "nuit"; 
  if(u.includes("JOUR")) return "jour";
...
function showImportPreview(svcs, ags, sel) {
  ...
  if(p&&s) {
    ...
    p.onchange=()=>{ 
      const ag=ags.find(x=>x.name===p.value); 
...
function renderPreviewList(svcs) {
  ...
  if(!l) return;
  if(!svcs.length) {l.innerHTML='<div class="preview-empty">Choisissez un agent dans la liste</div>';return;}
  
  l.innerHTML=svcs.map(s=>`
    <div class="preview-row ${s.status}">
      <div>${s.dateObj.getDate()}/${s.dateObj.getMonth()+1}</div>
      <div class="preview-code">${s.code}</div>
      <div class="preview-status">${STATUS_LABELS[s.status]}</div>
    </div>
  `).join('');
}
...
function updateImportSummary(svcs) {
  ...
  $('importSummary').textContent=`${svcs.length} services — ☀️${j} 🌙${n}`;
}
...
function openNoteModal() {
  ...
  $('sheetTitle').textContent=`Note du ${d.getDate()}`; 
...
function setupEvents() {
  ...
  $('btnApplyOther')?.addEventListener('click',()=>{
    const v=$('otherSelect').value; 
    saveEntry(state.selected,{status:'autre',custom_label:v});
    closeModal('backdrop','sheet');
  });
  ...
  $('fileInput')?.addEventListener('change',e=>{
    ...
    if(e.target.files[0]){
      ...
      r.onload=ev=>{
        ...
        let best=null,sc=-1;
        wb.SheetNames.forEach(n=>{
          const row=XLSX.utils.sheet_to_json(wb.Sheets[n],{header:1});
          const s=row.reduce((a,b)=>a+b.filter(c=>String(c).trim()).length,0);
          if(s>sc){sc=s;best=row;}
        });
  ...
  ['rateDay','rateNightFull','rateNightSolo','rateMN'].forEach(id=>{
    $(id)?.addEventListener('change',e=>{
      prefs[id]=parseFloat(e.target.value)||0;
      savePrefs();
      updateUI();
    });
  });
...
function openStats() {
  ...
  for (let i=1; i<=end.getDate(); i++) { 
    const e=entries.get(keyFor(new Date(state.year,state.month,i))); 
    if(e&&counts[e.status]!==undefined) counts[e.status]++; 
  }
    ...
    <div class="stats-grid">
      ${Object.entries(counts).map(([k,v])=>`
        <div class="stats-row ${k}">
          ...
          <span class="stats-name">${STATUS_LABELS[k]}</span>
          <span class="stats-count">${v}</span>
          <div class="stats-bar-mini">
            <div class="stats-bar-fill ${k}" style="width:${(v/end.getDate()*100).toFixed(0)}%"></div>
          </div>
        </div>
      `).join('')}
    </div>
    <div class="stats-summary">
      <div class="summary-row"><span>Total services</span><strong>${total}</strong></div>
      <div class="summary-row highlight"><span>Brut Est.</span><strong>${calculateSalary().toLocaleString('fr-FR',{minimumFractionDigits:2})} €</strong></div>
    </div>
  `;
